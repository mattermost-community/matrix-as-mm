...
export default class LoginPage {
    ...
    constructor(page: Page) {
        ...
        this.loginPlaceholder = page.locator(
        ...
        this.userErrorLabel = page.locator(
    ...
    async login(user: UserAuthProfile, useUsername = true) {
        ...
        const [response] = await Promise.all([
            this.page.waitForResponse(
                response =>
                    response.url().endsWith('/users/me/teams') && response.ok(),
            ),
        ...
        expect(
            this.teams.length,
            'User must be member of a team.',
        ).toBeGreaterThanOrEqual(1);
    }

    async logout() {
        await this.page
            .locator('#RightControlsContainer')
            .getByRole('img', { name: 'user profile image' })
            .click();
